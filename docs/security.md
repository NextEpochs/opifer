...
- Web search keys (Brave, Tavily, SearXNG URL) are company secrets like any other: stored encrypted, never returned by the API, read only at the moment of a search and logged. With ChatGPT or Anthropic models the search runs inside the provider: the query and the pages it opens are seen by that provider, not fetched by Opifer.
