...
const goalBody = {
  ...
  properties: { title: { type: "string", minLength: 1, maxLength: 300 }, description: { type: "string", maxLength: 5000 }, measure: { type: "string", maxLength: 1000 }, parentId: nullableUuid, dueAt: { type: ["string", "null"] } },
} as const;

const goalPatch = {
  type: "object",
  additionalProperties: false,
  properties: { title: { type: "string", minLength: 1, maxLength: 300 }, description: { type: "string", maxLength: 5000 }, measure: { type: "string", maxLength: 1000 }, parentId: nullableUuid, status: { type: "string", enum: ["active", "reached", "dropped"] }, dueAt: { type: ["string", "null"] } },
} as const;
...
const projectBody = {
  ...
  required: ["name"],
  additionalProperties: false,
  properties: { name: { type: "string", minLength: 1, maxLength: 200 }, description: { type: "string", maxLength: 5000 }, goalId: nullableUuid, workdir: { type: ["string", "null"], maxLength: 1000 } },
} as const;

const projectPatch = {
  type: "object",
  additionalProperties: false,
  properties: { name: { type: "string", minLength: 1, maxLength: 200 }, description: { type: "string", maxLength: 5000 }, goalId: nullableUuid, workdir: { type: ["string", "null"], maxLength: 1000 }, status: { type: "string", enum: ["active", "paused", "done", "archived"] } },
} as const;
...
export async function registerWorkRoutes(app: FastifyInstance, options: WorkRoutesOptions): Promise<void> {
  ...
  app.post<{ Params: { id: string }; Body: { title: string; description?: string; measure?: string; parentId?: string | null; dueAt?: string | null } }>("/companies/:id/goals", { schema: { body: goalBody } }, async (request, reply) =>
    handle(reply, async () => {
      const goal = await work.createGoal({ companyId: request.params.id, title: request.body.title, description: request.body.description ?? "", measure: request.body.measure ?? "", parentId: request.body.parentId ?? null, dueAt: dateOrNull(request.body.dueAt) ?? null }, person);
  ...
  app.patch<{ Params: { id: string; goalId: string }; Body: Record<string, unknown> }>("/companies/:id/goals/:goalId", { schema: { body: goalPatch } }, async (request, reply) =>
    handle(reply, async () => {
      const b = request.body as { title?: string; description?: string; measure?: string; parentId?: string | null; status?: "active" | "reached" | "dropped"; dueAt?: string | null };
      const { dueAt, ...rest } = b;
  ...
  app.post<{ Params: { id: string }; Body: { name: string; description?: string; goalId?: string | null; workdir?: string | null } }>("/companies/:id/projects", { schema: { body: projectBody } }, async (request, reply) =>
    handle(reply, async () => {
  ...
  app.patch<{ Params: { id: string; projectId: string }; Body: Record<string, unknown> }>("/companies/:id/projects/:projectId", { schema: { body: projectPatch } }, async (request, reply) =>
    handle(reply, async () => {
  ...
  app.post<{ Params: { id: string }; Body: { title: string; description?: string; acceptance?: string; priority?: TaskPriority; projectId?: string | null; goalId?: string | null; parentId?: string | null; assigneeAgentId?: string | null; reviewerAgentId?: string | null; dueAt?: string | null } }>(
    "/companies/:id/tasks",
    { schema: { body: taskBody } },
    async (request, reply) =>
      handle(reply, async () => {
  ...
  app.get<{ Params: { id: string } }>("/tasks/:id", async (request, reply) => {
    ...
    const [why, comments, products, children] = await Promise.all([work.whyChain(companyId, task), work.listComments(companyId, task.id), work.listProducts(companyId, task.id), work.listTasks(companyId, { parentId: task.id })]);
    const sessions = await sql<{ id: string; agent_id: string; status: string; created_at: Date }[]>`SELECT id, agent_id, status, created_at FROM sessions WHERE task_id = ${task.id} ORDER BY created_at`;
    const [cost] = await sql<{ eur: string; usd: string; calls: string }[]>`SELECT coalesce(sum(amount_eur), 0)::text AS eur, coalesce(sum(amount_usd), 0)::text AS usd, count(*)::text AS calls FROM cost_events WHERE task_id = ${task.id}`;
    return {
      ...serialize(task),
      why,
      comments,
  ...
  action<{ agentId?: string | null; userId?: string | null }>("assign", (c, id, b) => work.assign(c, id, { agentId: b?.agentId ?? null, userId: b?.userId ?? null }, person), { type: "object", additionalProperties: false, properties: { agentId: nullableUuid, userId: nullableUuid } });
  action<{ summary: string; verification?: string }>("complete", (c, id, b) => work.complete(c, id, b, person, { from: ["in_progress", "in_review", "todo", "blocked"] }), resultBody);
  action<{ note: string }>("request-changes", (c, id, b) => work.requestChanges(c, id, b.note, person), { type: "object", required: ["note"], additionalProperties: false, properties: { note: { type: "string", minLength: 1, maxLength: 5000 } } });
  action<{ reason: string }>("block", (c, id, b) => work.block(c, id, b.reason, person), { type: "object", required: ["reason"], additionalProperties: false, properties: { reason: { type: "string", minLength: 1, maxLength: 2000 } } });
  action<Record<string, never>>("unblock", (c, id) => work.unblock(c, id, person));
  action<{ reason?: string }>("cancel", (c, id, b) => work.cancel(c, id, b?.reason ?? "", person), { type: "object", additionalProperties: false, properties: { reason: { type: "string", maxLength: 2000 } } });
  action<{ reason?: string }>("release", async (c, id, b) => work.release(c, id, { kind: "paused", reason: b?.reason ?? "released by a person" }, person), { type: "object", additionalProperties: false, properties: { reason: { type: "string", maxLength: 2000 } } });
  ...
  app.post<{ Params: { id: string }; Body: { body: string } }>("/tasks/:id/comments", { schema: { body: { type: "object", required: ["body"], additionalProperties: false, properties: { body: { type: "string", minLength: 1, maxLength: 20_000 } } } } }, async (request, reply) =>
    handle(reply, async () => {
      const companyId = await companyOfTask(request.params.id);
      if (!companyId) return reply.code(404).send({ error: "task not found" });
  ...
  app.post<{ Params: { id: string }; Body: { kind: "file" | "link" | "diff" | "document" | "decision" | "note"; title: string; ref?: string; summary?: string } }>(
    "/tasks/:id/products",
    { schema: { body: { type: "object", required: ["kind", "title"], additionalProperties: false, properties: { kind: { type: "string", enum: ["file", "link", "diff", "document", "decision", "note"] }, title: { type: "string", minLength: 1, maxLength: 300 }, ref: { type: "string", maxLength: 20_000 }, summary: { type: "string", maxLength: 5000 } } } } },
    async (request, reply) =>
      handle(reply, async () => {
        const companyId = await companyOfTask(request.params.id);
        if (!companyId) return reply.code(404).send({ error: "task not found" });
  ...
  app.get<{ Params: { id: string }; Querystring: { status?: string } }>("/companies/:id/wakeups", async (request) => work.listWakeups(request.params.id, { ...(request.query.status ? { status: request.query.status as never } : {}) }));
}
