...
export async function registerGovernanceRoutes(app: FastifyInstance, options: GovernanceRoutesOptions): Promise<void> {
  ...
  app.put<{ Params: { id: string }; Body: { scopeKind: "company" | "project" | "agent" | "task" | "turn"; scopeId?: string; window?: "monthly" | "daily" | "lifetime"; cap: number; currency?: "EUR" | "USD"; warnRatio?: number } }>(
    "/companies/:id/budgets",
    { schema: { body: budgetBody } },
    async (request, reply) => {
      try {
  ...
  app.post<{ Params: { id: string }; Body: { status: "approved" | "denied"; note?: string; newCap?: number } }>("/approvals/:id/decide", { schema: { body: decideBody } }, async (request, reply) => {
    const [row] = await sql<{ company_id: string }[]>`SELECT company_id FROM approvals WHERE id = ${request.params.id}`;
    if (!row) return reply.code(404).send({ error: "approval not found" });
    ...
    if (approval.kind === "tool_use" || approval.kind === "dangerous_command") {
      ...
      if (policy) {
        ...
        await budget.setPolicy({ companyId, scopeKind: policy.scopeKind, scopeId: policy.scopeId, window: policy.window, cap, currency: policy.currency, warnRatio: policy.warnRatio });
  ...
  app.get<{ Params: { id: string }; Querystring: { agentId?: string } }>("/companies/:id/secret-bindings", async (request) => secrets.listBindings(request.params.id, request.query.agentId ?? null));

  app.post<{ Params: { id: string }; Body: { secretName: string; agentId: string; toolName?: string } }>("/companies/:id/secret-bindings", { schema: { body: bindingBody } }, async (request, reply) => {
    try {
      const binding = await secrets.bind({ companyId: request.params.id, secretName: request.body.secretName, agentId: request.body.agentId, toolName: request.body.toolName ?? null });
  ...
  app.patch<{ Params: { id: string }; Body: { name?: string; role?: string; model?: string | null; reportsToAgentId?: string | null; note?: string } }>("/agents/:id", { schema: { body: agentPatchBody } }, async (request, reply) => {
    const agent = await agentOf(request.params.id);
    if (!agent) return reply.code(404).send({ error: "agent not found" });
  ...
  app.post<{ Params: { id: string }; Body: { status: "active" | "paused" | "archived"; reason?: string } }>("/agents/:id/status", { schema: { body: agentStatusBody } }, async (request, reply) => {
    const agent = await agentOf(request.params.id);
    if (!agent) return reply.code(404).send({ error: "agent not found" });
