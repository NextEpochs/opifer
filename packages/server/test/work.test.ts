...
describe("Work: tasks, wake-ups and the scheduler", () => {
  ...
  beforeAll(async () => {
            ...
            calls: [
              {
                name: "task_deliver",
                arguments: {
                  summary: "Pricing page drafted with three tiers",
                  verification: "open pricing.md",
                  products: [{ kind: "file", title: "pricing.md", ref: "pricing.md" }],
                },
              },
            ],
          };
    ...
    nora = (
      (await app.inject({ method: "POST", url: `/v1/companies/${companyId}/agents`, payload: { name: "Nora", role: "Researcher", reportsToAgentId: philip } })).json() as {
        id: string;
      }
    ).id;
  }, 120_000);
  ...
  it("assigning a task wakes the agent, who checks it out, works with a heartbeat, asks approval, and delivers for review", async () => {
    const goal = (await app.inject({ method: "POST", url: `/v1/companies/${companyId}/goals`, payload: { title: "Launch the pricing page", measure: "page live" } })).json() as {
      id: string;
    };
    ...
    const created = await app.inject({
      ...
      payload: { title: "Draft the pricing page", acceptance: "a pricing.md with three tiers", projectId: project.id, assigneeAgentId: nora, priority: "high" },
    });
    ...
    await runScheduler();
    let detail = (await app.inject({ method: "GET", url: `/v1/tasks/${task.id}` })).json() as {
      status: string;
      leaseExpiresAt: string | null;
      leaseSessionId: string | null;
      sessions: Array<{ id: string }>;
      why: { goals: Array<{ title: string }>; project: { name: string } };
    };
    ...
    await runScheduler();
    detail = (await app.inject({ method: "GET", url: `/v1/tasks/${task.id}` })).json() as typeof detail & {
      result: { summary: string };
      products: Array<{ kind: string; title: string }>;
    };
    ...
    const done = await app.inject({
      ...
      payload: { summary: "Verified: pricing.md has three tiers", verification: "read the file" },
    });
  ...
  it("a person's comment wakes the assignee, who answers with a comment; changes requested send the task back", async () => {
    const created = (await app.inject({ method: "POST", url: `/v1/companies/${companyId}/tasks`, payload: { title: "Compare competitors", assigneeAgentId: nora } })).json() as {
      id: string;
    };
    ...
    const deferred = (await app.inject({ method: "GET", url: `/v1/companies/${companyId}/wakeups?status=pending` })).json() as Array<{
      reason: string;
      taskId: string;
      error: string | null;
    }>;
    ...
    const wakeups = (await app.inject({ method: "GET", url: `/v1/companies/${companyId}/wakeups?status=pending` })).json() as Array<{
      reason: string;
      taskId: string;
      payload: { changesRequested?: string };
    }>;
