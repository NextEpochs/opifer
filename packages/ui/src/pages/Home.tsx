...
import { api, type CostReport, type ModelsInfo } from "../api";
...
const ALL_WIDGETS: WidgetId[] = ["needsYou", "spend", "team", "done", "activity", "costByAgent", "models"];

const span: Record<WidgetSize, string> = { s: "xl:col-span-4", m: "xl:col-span-6", l: "xl:col-span-8" };
                ...
                {picker ? (
                  <Card className="p-4">
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                      {missing.map((id) => (
                        <button key={id} type="button" onClick={() => add(id)} className="flex flex-col gap-1 rounded-[14px] border border-line bg-raised p-3 text-left hover:border-accent hover:bg-hover">
                  ...
                  <button type="button" onClick={() => setPicker(true)} className="flex h-16 w-full items-center justify-center gap-2 rounded-card border-2 border-dashed border-line-strong text-sm font-bold text-mute hover:border-accent hover:text-ink">
...
function SortableWidget({ placement, title, t, onResize, onRemove, children }: { placement: WidgetPlacement; title: string; t: Workspace["t"]; onResize: (s: WidgetSize) => void; onRemove: () => void; children: ReactNode }) {
            ...
            <>
              <div role="group" aria-label={t.widgetSize} className="flex gap-0.5 rounded-md bg-raised p-0.5">
                {(["s", "m", "l"] as WidgetSize[]).map((s) => (
                  <button key={s} type="button" aria-pressed={placement.size === s} aria-label={t.sizes[s]} onClick={() => onResize(s)} className={`h-6 w-6 rounded text-[11px] font-bold uppercase ${placement.size === s ? "bg-hover text-ink" : "text-faint hover:text-ink"}`}>
                    {s}
                  </button>
                ))}
              </div>
              ...
              <button ref={setActivatorNodeRef} type="button" aria-label={t.dragToReorder} className="cursor-grab rounded p-1 text-faint hover:bg-hover hover:text-ink active:cursor-grabbing" {...attributes} {...listeners}>
  ...
  return (
    <div className="flex items-center gap-5 px-[18px] pb-[18px] pt-2">
      <svg viewBox="0 0 120 120" width="112" height="112" role="img" aria-label={`${money(eur, currency)} ${cap ? fill(t.ofCap, { cap: money(cap, currency, 0) }) : t.noCap}`}>
        ...
        {cap ? <circle cx="60" cy="60" r="48" fill="none" stroke={tone} strokeWidth="14" strokeDasharray={`${dash} 301`} strokeLinecap="round" transform="rotate(-90 60 60)" /> : null}
...
function Team({ ws }: { ws: Workspace }) {
  ...
  return (
    <div className="flex flex-col gap-2.5 px-[18px] pb-[18px] pt-2">
      ...
      <a href="#/team/new" className="mt-1 inline-flex h-9 items-center gap-2 self-start rounded-control border border-line-strong px-3 text-[13px] font-bold text-ink no-underline hover:bg-hover">
...
function Models({ ws }: { ws: Workspace }) {
  ...
  useEffect(() => {
    api.models().then(setInfo).catch(() => setInfo(null));
  }, []);
...
function CostByAgent({ ws }: { ws: Workspace }) {
  ...
  useEffect(() => {
    api.costs(company.id).then(setReport).catch(() => setReport(null));
      ...
      {report.byAgent.map((a) => (
        <div key={a.agentId ?? "none"} className="flex items-center gap-3">
          ...
          <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-raised">
            <div className="h-full rounded-full" style={{ width: `${Math.max(2, (a.eur / max) * 100)}%`, background: "var(--o-accent)" }} />
          </div>
