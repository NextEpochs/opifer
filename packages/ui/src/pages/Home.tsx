...
export function HomePage({ ws }: { ws: Workspace }) {
  ...
  const [layout, setLayout] = usePref<WidgetPlacement[]>(
    `layout.${company.id}`,
    DEFAULT_LAYOUT,
  );
  ...
  const add = (id: WidgetId) => {
    setLayout((current) =>
      current.some((w) => w.id === id)
        ...
        : [
            ...current,
            { id, size: id === "needsYou" || id === "done" ? "l" : "m" },
          ],
    );
    setPicker(false);
  };
  ...
  const firstAgent =
    overview?.agents.find((a) => a.status === "active") ?? overview?.agents[0];
      ...
      <header className="flex flex-wrap items-end justify-between gap-5">
        <div>
          <h1 className="m-0 font-display text-[34px] font-bold leading-[1.1] tracking-tight">
            {greeting(t, company.name, overview?.working ?? 0)}
          </h1>
        ...
        <div className="flex gap-2">
          {firstAgent && (
            <Button
              variant="ghost"
              onClick={() => ws.go("chat", `new-${firstAgent.id}`)}
            >
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
            {layout.map((w) => (
              <SortableWidget
                key={w.id}
                placement={w}
                title={t.widgets[w.id as WidgetId] ?? w.id}
                t={t}
...
function SortableWidget({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`${span[placement.size]} ${isDragging ? "z-10 opacity-90 shadow-lift" : ""}`}
    >
          ...
          setReviews(
            r
              .filter(
                (x) =>
                  x.status === "done" &&
                  ((x.applied.memoryIds?.length ?? 0) > 0 || x.applied.skill),
              )
              .slice(0, 6),
          ),
        )
    ...
    <div className="flex flex-col gap-2 px-[18px] pb-[18px] pt-2 text-sm">
      {reviews.map((r) => (
        <a
          key={r.id}
          href="#/learning"
          className="flex gap-2.5 rounded-control px-1 py-1 text-ink no-underline hover:bg-hover"
        >
  ...
  return (
    <div className="flex flex-col gap-3 px-[18px] pb-[18px] pt-3">
      ...
      {total > 3 && (
        <a
          href="#/inbox"
          className="text-[13px] font-bold text-accent-text no-underline"
        >
  ...
  return (
    <div className="flex items-center gap-5 px-[18px] pb-[18px] pt-2">
      <svg
        ...
        height="112"
        role="img"
        aria-label={`${money(eur, currency)} ${cap ? fill(t.ofCap, { cap: money(cap, currency, 0) }) : t.noCap}`}
      >
        <circle
          cx="60"
          cy="60"
      ...
      <div>
        ...
        <div className="mt-1.5 text-[13px] text-mute">
          {cap ? fill(t.ofCap, { cap: money(cap, currency, 0) }) : t.noCap}
        </div>
        ...
        <a
          href="#/money"
          className="mt-2 inline-block text-[13px] font-bold text-accent-text no-underline"
        >
          {t.seeMoney} →
        </a>
      </div>
...
function Team({ ws }: { ws: Workspace }) {
  ...
  return (
    <div className="flex flex-col gap-2.5 px-[18px] pb-[18px] pt-2">
      {agents.length === 0 && (
        <p className="m-0 text-sm text-mute">{t.nobodyWorking}</p>
      )}
      {agents.map((a) => (
        <a
          key={a.id}
          href={`#/team/${a.id}`}
          className="flex items-center gap-2.5 rounded-control px-1 py-1 text-ink no-underline hover:bg-hover"
        >
          <Avatar name={a.name} />
  ...
  return (
    <div className="flex flex-col gap-2 px-[18px] pb-[18px] pt-2 text-sm">
      {runs.map((r) => (
        ...
        >
          <span
            className={`font-extrabold ${r.status === "completed" ? "text-ok" : r.status === "failed" ? "text-danger" : "text-warn"}`}
            aria-hidden="true"
          >
          ...
          <span className="min-w-0">
            <strong>{ws.agentName(r.agentId)}</strong> ·{" "}
            {r.sessionTitle ?? t.untitled}
            <span className="block truncate text-[13px] text-mute">
              {r.preview}
            </span>
          </span>
          <span className="ml-auto shrink-0 text-[12px] text-faint">
            {timeAgo(r.finishedAt ?? r.startedAt, t)}
          </span>
        </a>
      ))}
    </div>
  ...
  return (
    <div className="flex flex-col gap-2 px-[18px] pb-[18px] pt-2 text-sm">
      <div>
        ...
        <strong className="font-mono text-[13px]">{info.default}</strong>
      </div>
    ...
    <div className="flex flex-col gap-2.5 px-[18px] pb-[18px] pt-2 text-sm">
      {report.byAgent.map((a) => (
        <div key={a.agentId ?? "none"} className="flex items-center gap-3">
          ...
          <span className="w-24 text-right font-mono text-[12px]">
            {money(a.eur, "EUR", 4)}
          </span>
