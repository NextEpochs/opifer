...
export function LearningPage({
  ...
  const agents = (overview?.agents ?? []).filter(
    (a) => a.status !== "archived",
  );
      ...
      <header className="flex flex-wrap items-end justify-between gap-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Segmented
            ...
            options={(["memory", "skills", "reviews", "settings"] as Tab[]).map(
              (v) => ({ value: v, label: t.learningTabs[v] }),
            )}
          />
        </div>
      </header>
      {tab === "memory" && (
        <MemoryTab ws={ws} agent={agent} companyId={company.id} />
      )}
      {tab === "skills" && (
        <SkillsTab ws={ws} agent={agent} companyId={company.id} />
      )}
...
function MemoryTab({
  ...
  const load = useCallback(async () => {
    ...
    const q = query.trim();
    if (q && agent)
      setList(await api.memories(companyId, { agent, q, status, limit: 40 }));
    ...
    () =>
      eventsSocket(
        (e) =>
          ...
          (e.type === "memory.saved" || e.type === "learning.reviewed") &&
          void load(),
        () => {},
      ),
    [companyId, load],
  ...
  const act = async (
    m: Memory,
    action: "pin" | "promote" | "correct" | "retire",
      ...
      {list.length === 0 ? (
        ...
        <ul className="m-0 grid list-none grid-cols-1 gap-3 p-0 xl:grid-cols-2">
          {list.map((m) => (
            <li
              key={m.id}
              className={`flex flex-col gap-2 rounded-[14px] border border-line bg-card p-4 ${m.status !== "active" ? "opacity-60" : ""}`}
            >
              <div className="flex items-start gap-3">
                <Avatar
                  name={
                    m.scope === "company"
                      ? ws.company.name
                      : ws.agentName(m.scopeAgentId)
                  }
                  size={30}
                />
                <div className="min-w-0 flex-1">
                  <p className="m-0 text-[14px] leading-snug">
                    {m.kind === "profile" && m.subject && (
                      <strong>{m.subject}: </strong>
                    )}
                    {m.content}
                  </p>
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-[12px] text-mute">
                    <Chip tone={m.scope === "company" ? "info" : "mute"}>
                      {t.scopeLabel[m.scope]}
                    </Chip>
                    {m.pinned && <Chip tone="warn">{t.pinned}</Chip>}
                    {m.status !== "active" && (
                      <Chip tone="mute">{m.status}</Chip>
                    )}
                    <span>
                      {m.authorKind === "agent"
                        ? ws.agentName(m.authorId)
                        : m.authorKind === "person"
                          ? t.you
                          : t.system}{" "}
                    ...
                    {m.sourceTaskId && (
                      <a
                        href={`#/work/${m.sourceTaskId}`}
                        className="font-bold text-accent-text no-underline"
                      >
                        {t.learnedFrom} →
                      </a>
                    )}
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    ...
                    {m.scope !== "company" && (
                      <button
                        ...
                        aria-label={t.share}
                        onClick={() =>
                          void act(m, "promote", { toScope: "company" })
                        }
                        className="rounded-md p-1.5 text-mute hover:bg-hover hover:text-ink"
                      >
                        <Share2 size={15} />
              ...
              {m.status === "active" && editing?.id !== m.id && (
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    onClick={() =>
                      setEditing({ id: m.id, mode: "correct", text: m.content })
                    }
                  >
                    {t.correct}
                  </Button>
                  <Button
                    ...
                    variant="ghost"
                    onClick={() =>
                      setEditing({ id: m.id, mode: "retire", text: "" })
                    }
                  >
                    {t.retire}
                  </Button>
              ...
              {editing?.id === m.id && (
                ...
                >
                  <Textarea
                    ...
                    placeholder={
                      editing.mode === "correct" ? t.correctHint : t.retireHint
                    }
                    aria-label={
                      editing.mode === "correct" ? t.correctHint : t.retireHint
                    }
        ...
        <div className="flex flex-wrap gap-3">
          <div className="w-64">
            <Input
              ...
              placeholder={t.memorySubject}
              aria-label={t.memorySubject}
            />
          </div>
          <label className="flex items-center gap-2 text-[13px] text-mute">
            {t.memoryFor}
...
function SkillsTab({
  ...
  const load = useCallback(async () => {
    setSkills(
      agent
        ? await api.skills(companyId, { agent })
        : await api.skills(companyId, { scope: "company" }),
    );
  }, [companyId, agent]);
  useEffect(() => {
  ...
  useEffect(
    () =>
      eventsSocket(
        (e) =>
          e.companyId === companyId &&
          (e.type === "skill.created" ||
            e.type === "learning.reviewed" ||
            e.type === "promotion.decided") &&
          void load(),
        () => {},
      ),
    [companyId, load],
  ...
  const scopeOf = (s: Skill) =>
    s.scope === "company"
      ? t.scopeLabel.company
      : s.scope === "team"
        ? `${t.scopeLabel.team} · ${ws.agentName(s.scopeAgentId)}`
        : ws.agentName(s.scopeAgentId);
              ...
              <li key={s.id}>
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[14px] font-bold text-accent-text">
                      {s.name}
                    </span>
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-[12px] text-mute">
                    <Chip tone={s.scope === "company" ? "info" : "mute"}>
                      {scopeOf(s)}
                    </Chip>
                    <Chip tone="mute">{t.skillOrigin[s.origin]}</Chip>
                    {s.status !== "active" && (
                      <Chip tone={s.status === "archived" ? "mute" : "warn"}>
                        {t.skillStatus[s.status]}
                      </Chip>
                    )}
...
function SkillDrawer({
  ...
  const load = useCallback(
    async () => setDetail(await api.skill(companyId, skillId)),
    [companyId, skillId],
  );
  useEffect(() => {
    setEditing(null);
  ...
  const run = async (fn: () => Promise<unknown>, message?: string) => {
    try {
      ...
      else if (r && typeof r === "object" && "status" in r)
        setNotice(
          (r as { status: string }).status === "applied"
            ? t.shared
            : t.proposedShare,
        );
      await load();
  ...
  const shown =
    viewing ??
    (s.version
      ? { version: s.version.version, content: s.version.content }
      : null);
  return (
    <aside
      className="flex w-[520px] shrink-0 flex-col rounded-card border border-line bg-panel shadow-card"
      aria-label={s.name}
    >
      <header className="flex items-start gap-3 border-b border-line px-5 py-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h2 className="m-0 font-mono text-[18px] font-bold text-accent-text">
              {s.name}
            </h2>
            <span className="text-[12px] text-mute">v{s.currentVersion}</span>
            {s.pinned && <Chip tone="warn">{t.pinned}</Chip>}
            {s.status !== "active" && (
              <Chip tone="mute">{t.skillStatus[s.status]}</Chip>
            )}
          </div>
          <p className="m-0 mt-1 text-[13px] text-mute">{s.description}</p>
          <p className="m-0 mt-1 text-[12px] text-mute">
            {t.skillOrigin[s.origin]} · {fill(t.uses, { n: s.uses })} ·{" "}
            {fill(t.worked, { ok: s.usage.successes, ko: s.usage.failures })}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex-1 space-y-4 overflow-auto px-5 py-4 text-sm">
        {notice && (
          <p className="m-0 rounded-control border border-line bg-card px-3 py-2 text-[13px]">
            {notice}
          </p>
        )}
        {editing ? (
          ...
          >
            ...
            <Input
              ...
              placeholder={t.noteForVersion}
              aria-label={t.noteForVersion}
            />
            <div className="flex gap-2">
              <Button
                type="submit"
          ...
          shown && (
            <div>
              ...
              <pre className="m-0 whitespace-pre-wrap rounded-[10px] border border-line bg-bg px-3 py-2.5 font-mono text-[12.5px] leading-relaxed text-ink">
                {shown.content}
              </pre>
            </div>
          )
        ...
        <section>
          <h3 className="m-0 mb-1.5 text-[12px] font-bold uppercase tracking-wide text-mute">
            {t.versions}
          </h3>
            ...
            {s.versions.map((v) => (
              <li key={v.version} className="flex items-center gap-2">
                <button
                  ...
                  onClick={() =>
                    void api
                      .skillVersion(companyId, s.id, v.version)
                      .then((x) =>
                        setViewing({ version: x.version, content: x.content }),
                      )
                  }
                ...
                <span className="min-w-0 flex-1 truncate text-mute">
                  {v.note || "—"} ·{" "}
                  {v.createdByKind === "agent"
                    ? ws.agentName(s.scopeAgentId)
                    : v.createdByKind === "person"
                      ? t.you
                      : t.system}{" "}
                ...
                {v.version !== s.currentVersion && (
                  <button
                    ...
                    onClick={() =>
                      void run(
                        () => api.restoreSkill(companyId, s.id, v.version),
                        t.restored,
                      )
                    }
        ...
        {s.status === "archived" && (
          <p className="m-0 text-[12px] text-mute">{t.archivedHint}</p>
        )}
      </div>
      <footer className="flex flex-wrap gap-2 border-t border-line px-5 py-3">
        {!editing && s.status !== "archived" && (
          <Button
            size="sm"
            onClick={() =>
              setEditing({ content: s.version?.content ?? "", note: "" })
            }
          >
            {t.improve}
          </Button>
        ...
        <Button
          ...
          onClick={() =>
            void run(
              () => api.pinSkill(companyId, s.id, !s.pinned),
        ...
        {s.scope !== "company" && s.status !== "archived" && (
          <Button
            size="sm"
            onClick={() => void run(() => api.promoteSkill(companyId, s.id))}
          >
...
function ReviewsTab({ ws, companyId }: { ws: Workspace; companyId: string }) {
  ...
  const load = useCallback(
    async () => setReviews(await api.learningReviews(companyId)),
    [companyId],
  );
  useEffect(() => {
  ...
  if (reviews.length === 0)
    return <p className="text-sm text-mute">{t.reviewsEmpty}</p>;
  return (
          ...
          r.status === "done"
            ? (r.applied.memoryIds?.length ?? 0) > 0 || r.applied.skill
              ? fill(t.reviewKept, {
                  ...
                  skill: r.applied.skill
                    ? fill(t.reviewSkill, { name: r.applied.skill.name })
                    : "",
                })
              : t.reviewNothing
            : (r.error ?? "");
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[14px]">
                ...
                {r.proposals.reason && r.status === "done" ? (
                  <span className="text-mute"> — {r.proposals.reason}</span>
                ) : null}
              </div>
...
function SettingsTab({ ws, companyId }: { ws: Workspace; companyId: string }) {
  ...
  if (!s) return null;
  const patch = async (p: Partial<Omit<LearningSettings, "semanticSearch">>) =>
    setS(await api.updateLearningSettings(companyId, p));
  return (
    <Card className="max-w-2xl p-5">
      <div className="flex flex-col gap-4 text-sm">
        <label className="flex items-center gap-3">
          <input
            type="checkbox"
            checked={s.reviewEnabled}
            onChange={(e) => void patch({ reviewEnabled: e.target.checked })}
          />
          {t.ruleReview}
        </label>
        ...
        <label className="flex items-center gap-3">
          <div className="w-20">
            <Input
              ...
              onChange={(e) =>
                void patch({ promotionThreshold: Number(e.target.value) || 1 })
              }
        ...
        <label className="flex items-center gap-3">
          <div className="w-28">
            <Input
              ...
              onChange={(e) =>
                void patch({ snapshotMaxChars: Number(e.target.value) || 6000 })
              }
        ...
        <p className="m-0 text-mute">
          {s.semanticSearch ? t.semanticOn : t.semanticOff}
        </p>
      </div>
    </Card>
  );
