...
export class LearningService {
  ...
  attachGovernance(governance: {
    ...
    this.reviewer = new Reviewer(
      ...
      this.skills,
      this.settings,
      this.reviewerOptions(this.options),
    );
  }
  ...
  async snapshot(
    ...
    const memory = await this.memories.snapshot(
      companyId,
      agentId,
      settings.snapshotMaxChars,
    );
  ...
  async onTaskClosed(
    ...
    if (outcome === "success")
      await this.promotions.proposeEligible(companyId, taskId);
  }
}
